...
//  expressions ~log2(n) times, where n is the number of rows.
//
...
//   accumulated state is simply appended to the list of initial states above.
//
...
//
//   This implementation converts all inputs into states by applying
//   inputFunction to input and initialValue, then combines multiple pairs of
//   states similar to global aggregation.
//
//   [s1, s2, s3, s4, s5] = f([s0, s0, s0, s0, s0], [1, 10, 2, 20, 3])
//
//   [s1, s2] = g([s1, s2], [s3, s4]) // s5 is unmodified
//
//   Now we have 2 states: s1, s5 for group 1 and s2 for group 2. Group 2 is
//   done. We proceed to combine states for group 1.
//
//   [s1] = g([s1], [s5])
//
//   Here, inputFunction lambda expression is evaluated once and
...
//
// Note that this more efficient algorithm doesn't support applying reduce_agg
// to sorted inputs.
//
