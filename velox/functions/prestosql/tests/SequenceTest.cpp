...
TEST_F(SequenceTest, testNegative) {
  const auto startVector = makeFlatVector<int64_t>({-1, -2, -10});
  const auto stopVector = makeFlatVector<int64_t>({-2, -5, -9});
  VectorPtr expected =
      makeArrayVector<int64_t>({{-1, -2}, {-2, -3, -4, -5}, {-10, -9}});
...
TEST_F(SequenceTest, testConstant) {
...
TEST_F(SequenceTest, testExceedMaxEntries) {
...
TEST_F(SequenceTest, testInvalidStep) {
