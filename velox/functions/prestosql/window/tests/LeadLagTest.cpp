...
TEST_P(LeadLagTest, smallPartitions) {
  ...
  // Small partitions.
  queryInfo = buildWindowQuery({data}, fn("c0"), "partition by c1", "");
  assertQuery(queryInfo.planNode, queryInfo.querySql);

  queryInfo = buildWindowQuery({data}, fn("c0, 1, 100"), "partition by c1", "");
  assertQuery(queryInfo.planNode, queryInfo.querySql);

  queryInfo = buildWindowQuery({data}, fn("c0, 2, c2"), "partition by c1", "");
  assertQuery(queryInfo.planNode, queryInfo.querySql);
}
