 ...
 private:
  ...
  struct SourceCursor {
    SourceCursor(
        ...
        : comparator_{comparator}, source_{source}, blockingFutures_{blockingFutures} {
      fetchMoreData();
    }
    ...
    void next() {
      current_ = {data_.get(), index_};

      ++index_;
      if (index_ == data_->size()) {
        // Hold on to a shared pointer to data_ to ensure current_ stays valid.
    ...
    bool operator<(const SourceCursor& other) const {
      if (atEnd_) {
        return false;
      }
      if (other.atEnd_) {
      ...
      return comparator_(*current(), *other.current());
    }
  ...
  std::unique_ptr<TreeOfLosers<SourceCursor>> treeOfLoosers_;
