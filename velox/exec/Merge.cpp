...
BlockingReason Merge::isBlocked(ContinueFuture* future) {
  ...
  if (sourceCursors_.empty()) {
    ...
    for (auto& source : sources_) {
      sourceCursors.push_back(std::make_unique<SourceCursor>(
          source.get(), sourceBlockingFutures_, comparator_));
      sourceCursors_.push_back(sourceCursors.back().get());
    }

    treeOfLoosers_ =
        std::make_unique<TreeOfLosers<SourceCursor>>(std::move(sourceCursors));
...
RowVectorPtr Merge::getOutput() {
  ...
  for (;;) {
    auto source = treeOfLoosers_->next();
