...
std::optional<int32_t> compare<TypeKind::ROW>(
  ...
  for (auto i = 0; i < childrenSize; ++i) {
    ...
    if (auto result = compareSwitch(left, *child, wrappedIndex, flags)) {
      return result;
    }
  }
  return 0;
...
std::optional<int32_t> compareArrays(
  ...
  for (auto i = 0; i < compareSize; ++i) {
    ...
    if (auto result =
            compareSwitch(left, *wrappedElements, elementIndex, flags)) {
      return result;
    }
  }
...
std::optional<int32_t> compare<TypeKind::MAP>(
  ...
  if (auto result = compareArrayIndices(left, *map->mapKeys(), rightIndices, flags)) {
    return result;
  }
