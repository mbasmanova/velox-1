...
struct MaterializedRowEpsilonComparator {
 public:
  ...
  /// precision for floating-point columns. Comparing with epsilon
  /// is supported only when non-floating-point columns form unique key in both
  /// expected and actual datasets. Returns std::nullopt if epsilon comparison
  /// is not possible. The caller may fall back on exact comparison in that
  /// case.
  ...
  /// sorting by non-floating point columns. Should be called only after
...
std::pair<uint32_t, std::vector<velox::column_index_t>>
...
bool assertEqualResults(
  ...
  if (numFloatingPointColumns) {
    ...
    auto result = comparator.areEqual(expectedRows, actualRows);
    if (result.has_value()) {
      if (!result.value()) {
        auto diffMessage = comparator.getUserFriendlyDiff();
...
static bool compareOrderedPartitions(
  ...
  auto [numFloatingPointColumns, columns] =
      findFloatingPointColumns(*left.second.begin());
  if (numFloatingPointColumns) {
    MaterializedRowEpsilonComparator comparator{
        numFloatingPointColumns, columns};
    auto result = comparator.areEqual(left.second, right.second);
    if (result.has_value()) {
      return result.value();
    }
  }
