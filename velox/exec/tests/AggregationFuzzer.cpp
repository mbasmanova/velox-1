...
void AggregationFuzzer::go() {
  ...
  while (!isDone(i, startTime)) {
    ...
    statistics_.functionNames.insert(signature.name);
    ...
    if (vectorFuzzer_.coinToss(0.1)) {
      ++statistics_.numGlobal;
    } else {
      ++statistics_.numGroupBy;
...
std::optional<MaterializedRowMultiset> computeExpectedResults(
    const std::vector<std::string>& groupingKeys,
    const std::vector<std::string>& aggregates,
    const std::vector<std::string>& masks,
    const std::vector<RowVectorPtr>& input,
...
void AggregationFuzzer::verify(
  ...
  try {
    ...
    if (resultOrError.exceptionPtr) {
      ++statistics_.numFailed;
    }

    std::optional<MaterializedRowMultiset> expectedResult;
    try {
      expectedResult = computeExpectedResults(
          groupingKeys, aggregates, masks, input, plan->outputType());
      ++statistics_.numDuckDbVerified;
    } catch (std::exception& e) {
...
void AggregationFuzzer::Statistics::print(size_t numIterations) const {
